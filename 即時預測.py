...
def main(): # 定義主函式，作為整個預測系統的入口點，負責協調各個模組的運行流程，從資料準備、特徵建立、模型訓練與回測，到最終的預測與報告生成
    ...
    for oil in oil_types: # 迭代 oil_types 列表中的每一個油品類型，對每個油品進行分析和預測，這樣可以確保在後續的分析和報告生成中涵蓋所有指定的油品類型，提供全面的分析結果
        ...
        pred_ai_raw, bundle = hybrid_predict_value( 
            train_data, 
            df_decision.tail(1), # 只取最後一行的特徵值來進行預測，這樣可以確保在進行下週最終預測時使用最新的特徵數據來生成預測結果，提供一個全面的分析結果來驗證模型的預測能力 
        ...
        if len(hist_vol_series.dropna()) > 20: # 如果歷史波動率序列中非 NaN 的值多於 20 個，則計算高波動率閾值和低波動率閾值，這樣可以在進行體制轉換決策時使用這些閾值來判斷當前的市場狀態，提供一個全面的分析結果來驗證模型的預測能力，同時幫助識別可能存在的市場波動影響問題
            ...
            VOL_THRESHOLD_LOW = hist_vol_series.quantile(0.15) # 計算低波動率閾值，這樣可以在進行體制轉換決策時使用這個閾值來判斷當前的市場狀態，提供一個全面的分析結果來驗證模型的預測能力，同時幫助識別可能存在的市場波動影響問題 
        else:
        ...
        if current_vol > VOL_THRESHOLD_HIGH: # 如果當前的波動率大於高波動率閾值，則認為市場處於高波動狀態，這樣可以在進行體制轉換決策時使用這個條件來判斷當前的市場狀態，提供一個全面的分析結果來驗證模型的預測能力，同時幫助識別可能存在的市場波動影響問題
            ...
            p_theoretical = 0.60 * pred_ai_raw + 0.40 * p_diff_arima # 在低波動狀態下，將 AI 預測的權重設置為 10%，ARIMA 預測的權重設置為 90%，這樣可以在進行體制轉換決策時充分利用 ARIMA 模型的預測能力來應對低波動的市場狀態，提供一個全面的分析結果來驗證模型的預測能力，同時幫助識別可能存在的市場波動影響問題
            ...
            w_ai = 0.60 + ratio * (0.35) # 在中波動狀態下，根據當前波動率的位置比例來動態調整 AI 預測的權重，這樣可以在進行體制轉換決策時充分利用 AI 模型的預測能力來應對不同程度的市場波動狀態，提供一個全面的分析結果來驗證模型的預測能力，同時幫助識別可能存在的市場波動影響問題
        ...
        # =====================================================
        # 🔥 (已刪除) 公式防呆守門員 (Formula Guardrail) 已被移除，讓 AI 發揮最大潛力
        # =====================================================

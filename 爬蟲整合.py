...
def build_refined_dataset(asia_realtime=None):  # 定義主資料集建構函式，負責調度爬蟲與特徵
    ...
    tickers = { 
        "BZ=F": '布蘭特原油', "CL=F": 'WTI原油', # 🔥 修正為 Yahoo Finance 的 WTI 原油代碼
    ...
    for ticker_code, name in tickers.items():
        ...
        # --- 嘗試 2：啟動終極官方備援機制 (FRED 官方 API) ---
        if price_df is None or price_df.empty:
            fred_map = {
                "BZ=F": "DCOILBRENTEU", # 布蘭特原油現貨
                "CL=F": "DCOILWTICO",   # 🔥 加入 WTI 的 FRED 備援代碼
                "USDTWD=X": "DEXTAUS",  # 台幣匯率
                "JPY=X": "DEXJPUS",    # 日圓匯率
                "KRW=X": "DEXKOUS",    # 韓元匯率
                "^VIX": "VIXCLS"       # 恐慌指數
            }
            
            fred_id = fred_map.get(ticker_code)
            # 從 GitHub Actions 的環境變數中讀取金鑰
            api_key = os.environ.get("FRED_API_KEY") 
            
            if not api_key:
                logging.error(f"   ❌ 找不到 FRED_API_KEY，請確認 GitHub Secrets 與 .yml 設定正確。")
            elif fred_id:
                logging.warning(f"   ⚠️ 啟動美國聯準會(FRED)官方 API 救援: {name} ({fred_id})...")
                try:
                    # FRED API 需要 YYYY-MM-DD 格式的日期
                    start_str = pd.to_datetime(start_date).strftime('%Y-%m-%d')
                    
                    # 組合官方 API 網址，要求回傳乾淨的 JSON 格式
                    fred_api_url = f"https://api.stlouisfed.org/fred/series/observations?series_id={fred_id}&api_key={api_key}&file_type=json&observation_start={start_str}"
                    
                    res = session.get(fred_api_url, timeout=15)
                    ...
                    if res.status_code == 200:
                        ...
                        if observations:
                            ...
                            # FRED 的缺失值會標記為 '.'，強制轉為數值，無法轉換的會變成 NaN
                            df_fred['value'] = pd.to_numeric(df_fred['value'], errors='coerce')
                            df_fred = df_fred.dropna(subset=['value'])
                            df_fred['date'] = pd.to_datetime(df_fred['date'])
                            
                            if not df_fred.empty:
                                price_df = df_fred[['date', 'value']].copy()
                                price_df.columns = ['日期', name]
                                logging.info(f"   ✅ FRED API 救援大成功: {name}")
                            else:
                                logging.warning(f"   ⚠️ FRED API 有回應，但過濾缺失值後無有效資料")
                        else:
                            logging.warning(f"   ⚠️ FRED API 回傳成功，但 observations 為空")
                    else:
                        logging.error(f"   ❌ FRED API 回傳異常 (狀態碼: {res.status_code}, 訊息: {res.text})")
                except Exception as e:
                    logging.error(f"   ❌ FRED API 備用源發生錯誤: {e}")
    ...
    # 🔥 [新增防呆] 確保核心特徵存在，否則安全撤退
    if '布蘭特原油' not in df_full.columns:
        print("❌ 致命錯誤：布蘭特原油資料獲取失敗，終止本次特徵工程。")
        return pd.DataFrame(columns=['日期', '布蘭特原油']) # 回傳空表讓系統安全結束
        
    if '台幣匯率' not in df_full.columns:
        df_full['台幣匯率'] = 32.0 # 假設極端情況下沒抓到匯率，給個預設值保命
        print("⚠️ 警告：缺少台幣匯率，使用預設值 32.0")

    # ==============================================================================
    # 🔥 新增：建立中油專屬 7D3B 購油成本特徵 (用 WTI 替代杜拜)
    # ==============================================================================
    if 'WTI原油' in df_full.columns and '布蘭特原油' in df_full.columns:
        # 前值填充空值，避免因某商品單日休市導致整列計算變成 NaN
        df_full['WTI原油'] = df_full['WTI原油'].ffill()
        df_full['布蘭特原油'] = df_full['布蘭特原油'].ffill()
        df_full['7D3B_指標'] = (df_full['WTI原油'] * 0.7) + (df_full['布蘭特原油'] * 0.3)
    else:
        df_full['7D3B_指標'] = df_full['布蘭特原油']
        print("⚠️ 缺少 WTI 原油，降級使用 100% 布蘭特原油計算指標")

    # 算出真正的中油進口成本
    df_full['中油購油成本'] = df_full['7D3B_指標'] * df_full['台幣匯率']
    
    # ==============================================================================
    # 🔥 終極修正：實作中油「週均價」比較機制
    # ==============================================================================
    # 1. 先計算過去 5 天的平均成本 (代表本週均價)
    weekly_avg_cost = df_full['中油購油成本'].rolling(window=5, min_periods=1).mean()
    
    # 2. 拿「本週均價」減去「上週均價」(往前推 5 天的那個均價)
    df_full['成本週變動'] = weekly_avg_cost.diff(periods=5).fillna(0)

    # --- 🔥 [關鍵修改] 讓亞鄰價格 "貼近" 成本，製造壓力 --- 
    df_full['temp_cost'] = df_full['中油購油成本'] # 改用更精準的 7D3B 成本來推算日韓價格
    ...
    # ==============================================================================
    theoretical_cost_twd = df_full['中油購油成本'] # 🔥 直接使用剛剛算好的 7D3B 精準成本

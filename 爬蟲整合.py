...
import pandas_ta as ta  # 載入 pandas_ta 套件，專門用來計算金融技術指標 (如 RSI、布林通道)
...
# ==========================================
def fetch_single_ticker_safe(ticker_info, start_date):  # 定義函式，安全地從 Yahoo Finance 下載單一商品資料
    """  # 多行註解開頭
    單一下載函式 (安全版)  # 說明此函式用途
    """  # 多行註解結尾
    ticker, name = ticker_info  # 將傳入的 tuple 解包成商品代碼 (ticker) 與自訂名稱 (name)
    print(f"   ⏳ 正在下載: {name} ({ticker})...")  # 印出下載提示訊息
    
    try:  # 嘗試下載
        # 建立一個偽裝成 Chrome 瀏覽器的 Session
        session = requests.Session()
        session.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
        })
        
        # 使用這個偽裝 session 來下載資料 (多加了 session 和 proxy 參數)
        new_data = yf.download(ticker, start=start_date, progress=False, auto_adjust=False, timeout=15, session=session, proxy=None) # 使用 yfinance 下載資料，關閉進度條，設定超時為 15 秒
        if new_data.empty:   # 如果下載下來的資料表是空的
            print(f"   ⚠️ 下載為空: {name}")  # 印出警告訊息
            return None  # 回傳空值

        # --- 1. 欄位選取邏輯 ---  # 步驟說明：處理 yfinance 回傳的複雜欄位結構
        price_data = None  # 初始化一個變數準備存放價格序列
        if isinstance(new_data.columns, pd.MultiIndex):  # 如果欄位是多層次索引 (MultiIndex，近期 yfinance 更新常見)
            try:  # 嘗試在多層索引中找尋收盤價
                if 'Close' in new_data.columns.get_level_values(0):  # 如果 'Close' 在第一層索引
                    price_data = new_data['Close']  # 提取 'Close' 底下的資料
                elif 'Close' in new_data.columns.get_level_values(1):  # 如果 'Close' 在第二層索引
                    price_data = new_data.xs('Close', level=1, axis=1)  # 透過截面 (xs) 提取第二層的 'Close' 資料
                else:  # 如果都找不到 'Close'
                    price_data = new_data.iloc[:, 0]  # 沒辦法，直接拿第一欄作為備案
            except:  # 如果找尋過程出錯
                 price_data = new_data.iloc[:, 0]  # 同樣拿第一欄作為備案
        elif 'Close' in new_data.columns:  # 如果是一般單層索引且有 'Close' 欄位
            price_data = new_data['Close']  # 提取 'Close' 欄位
        elif 'Adj Close' in new_data.columns:  # 如果沒有 'Close' 但有 'Adj Close' (還原收盤價)
             price_data = new_data['Adj Close']  # 提取 'Adj Close' 欄位
        else:  # 如果都沒有
             price_data = new_data.iloc[:, 0]  # 硬塞第一欄作為備案

        # --- 2. 型別統一轉 DataFrame ---  # 步驟說明：確保格式一致
        if isinstance(price_data, pd.Series):  # 如果取出來的資料是單維度的 Series
            price_df = price_data.to_frame(name=name)  # 將其轉換為 DataFrame 並以商品自訂名稱命名欄位
        else:  # 如果已經是 DataFrame
            price_df = price_data.copy()  # 複製一份
            if price_df.shape[1] > 1:  # 如果欄位超過一欄 (可能剛剛抓到多個商品)
                price_df = price_df.iloc[:, :1]  # 強制只保留第一欄
            price_df.columns = [name]  # 將欄位重新命名為商品自訂名稱

        # 3. 索引與日期處理  # 步驟說明
        price_df.index.name = '日期'  # 將索引 (Index) 命名為 '日期'
        price_df = price_df.reset_index()  # 將索引重置為一般欄位，原本的索引變成名為 '日期' 的欄位
        price_df['日期'] = pd.to_datetime(price_df['日期'])  # 確保 '日期' 欄位為標準時間格式
        
        # 4. 防呆檢查  # 步驟說明：過濾極端異常值
        last_val = price_df[name].iloc[-1]  # 取得資料最新一天（最後一筆）的數值
        if name == '布蘭特原油':  # 如果現在抓的是布蘭特原油
            if last_val > 200 or last_val < 10: return None  # 油價大於200或小於10極可能是資料源錯誤，捨棄不採用
        elif name == '台幣匯率':  # 如果現在抓的是台幣匯率
            if last_val > 50 or last_val < 20: return None  # 匯率超出 20~50 範圍極可能是錯誤，捨棄不採用
                
        print(f"   ✅ 下載成功: {name} (最新值: {last_val:.2f})")  # 檢查過關，印出成功訊息與最新數值
        return price_df  # 回傳處理好的 DataFrame

    except Exception as e:  # 捕捉下載或處理過程中發生的任何錯誤
        print(f"   ❌ 下載失敗 {name}: {e}")  # 印出失敗訊息
        return None  # 回傳空值
...
# ==========================================
def add_technical_features(df, price_col='布蘭特原油'):  # 定義函式，加入常用的技術分析指標
    df = df.copy()  # 複製資料表，不更動原始資料
    df[price_col] = pd.to_numeric(df[price_col], errors='coerce')  # 確保基準價格欄位是數值型態
    
    high = df['High'] if 'High' in df.columns else df[price_col]  # 如果有最高價欄位就用它，沒有就用基準價代替 (方便計算 ATR)
    low = df['Low'] if 'Low' in df.columns else df[price_col]  # 如果有最低價欄位就用它，沒有就用基準價代替
    
    # 1. 必備技術指標  # 步驟說明
    df['ATR'] = ta.atr(high, low, df[price_col], length=14)  # 使用 pandas_ta 計算 14 期的真實真實波幅 (ATR，代表波動度)
    bb = ta.bbands(df[price_col], length=20, std=2)  # 計算 20 期、2 個標準差的布林通道 (Bollinger Bands)
    if bb is not None: df['BB_WIDTH'] = bb.iloc[:, 2]  # 如果布林通道計算成功，取第 3 欄 (通道寬度帶 Bandwidth) 存入特徵
    df['RSI'] = ta.rsi(df[price_col], length=14)  # 計算 14 期的相對強弱指數 (RSI，判斷超買超賣)

    # 2. 波動相關 (Momentum_Vol & Panic_Sell 需要)  # 步驟說明
    # 雖然模型不直接吃 Vol_Short，但它是中間變數，必須保留  # 開發者註解
    df['Vol_Short'] = df[price_col].pct_change().rolling(5).std()  # 計算 5 日報酬率的標準差，作為短期波動率特徵
    
    # 3. 組合特徵 (XGBoost 核心特徵)  # 步驟說明：建立複合型特徵以增強樹狀模型表現
    if '恐慌指數' in df.columns and '台幣匯率' in df.columns:  # 如果資料表中有這兩個欄位
        df['VIX_x_USD'] = df['恐慌指數'] * df['台幣匯率']  # 將恐慌指數(VIX)與匯率相乘，捕捉「全球恐慌+資金回流美元」的雙重壓力
        
    if 'RSI' in df.columns:  # 確保 RSI 欄位存在
        # Panic_Sell = 波動率 * (100 - RSI) -> RSI 低且波動大時觸發  # 說明這項指標背後的金融邏輯
        vol = df['Vol_Short']  # 取得短期波動率
        df['Panic_Sell'] = vol * (100 - df['RSI'])  # 建立「恐慌拋售」指標：當市場大跌(RSI低)且波動極大時數值會飆高
    
    # 4. 趨勢乖離 (MA5_Bias)  # 步驟說明
    ma5 = df[price_col].rolling(5).mean()  # 計算 5 日移動平均線 (週線)
    df['MA5_Bias'] = (df[price_col] / (ma5 + 1e-9)) - 1  # 計算當前價格偏離 5 日均線的幅度 (+1e-9 避免除以零)

    # 5. 動能波動 (Momentum_Vol)  # 步驟說明
    df['Momentum_Vol'] = df[price_col].diff() * (df['Vol_Short'] + 1e-9)  # 價格變動量乘以波動率，包含方向(正負)與力道的複合特徵
    
    # 6. 週期特徵 (Sin/Cos)  # 步驟說明：用三角函數處理時間週期，讓模型理解「距離年底還有多久」這種循環性
    if not np.issubdtype(df['日期'].dtype, np.datetime64):  # 檢查日期欄位型態
        df['日期'] = pd.to_datetime(df['日期'])  # 強制轉為時間型態
    day_of_year = df['日期'].dt.dayofyear  # 取出該日期是當年的第幾天 (1~365)
    df['sin_365'] = np.sin(2 * np.pi * day_of_year / 365.25)  # 轉換成年週期的 sin 特徵 (365.25 考慮閏年)
    df['cos_365'] = np.cos(2 * np.pi * day_of_year / 365.25)  # 轉換成年週期的 cos 特徵 (讓時間變成一個圓，12/31 跟 1/1 是相鄰的)
    df['sin_90'] = np.sin(2 * np.pi * day_of_year / 91.3)  # 轉換成季週期 (約 90 天) 的 sin 特徵
    df['cos_90'] = np.cos(2 * np.pi * day_of_year / 91.3)  # 轉換成季週期 (約 90 天) 的 cos 特徵
    
    return df  # 回傳加上滿滿特徵的資料表
...
def build_refined_dataset(asia_realtime=None):  # 定義主資料集建構函式，負責調度爬蟲與特徵
    ...
    # 2. 下載新資料 (🔥 移除 WTI，因為沒用到)  # 步驟說明
    tickers = {  # 定義一個字典，存放要抓取的 Yahoo 代碼與對應的中文名稱
        "BZ=F": '布蘭特原油', 
        "USDTWD=X": '台幣匯率', "JPY=X": '日圓匯率', 
        "KRW=X": '韓元匯率', "^VIX": '恐慌指數'
    }
    new_data_frames = []  # 準備存放抓下來的各個商品資料表
    
    for ticker_code, name in tickers.items():  # 逐一巡覽目標商品
        time.sleep(1)  # 暫停 1 秒，避免密集發送請求被 Yahoo 封鎖
        res = fetch_single_ticker_safe((ticker_code, name), start_date)  # 呼叫自訂的安全爬蟲函式抓取資料
        if res is not None:  # 如果成功抓到
            new_data_frames.append(res)  # 加入列表中

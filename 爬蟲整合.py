        ...
        if price_df is None or price_df.empty:
            ...
            try:
                ...
                if res.status_code == 200 and "Date" in res.text:
                    ...
                    # 過濾起始日期
                    df_stooq = df_stooq[df_stooq['Date'] >= pd.to_datetime(start_date)]
                    
                    if not df_stooq.empty:
                        # 整理成與 Yahoo 相同的格式
                        price_df = df_stooq[['Date', 'Close']].copy()
                        price_df.columns = ['日期', name]
                        logging.info(f"   ✅ Stooq 備用源救援成功: {name}")
                    else:
                        logging.warning(f"   ⚠️ Stooq 有回應，但期間內無資料")
                else:
                    logging.error(f"   ❌ Stooq 回傳異常 (狀態碼: {res.status_code})")
            except Exception as e:

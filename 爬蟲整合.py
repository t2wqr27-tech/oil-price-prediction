...
def build_refined_dataset(asia_realtime=None):  # 定義主資料集建構函式，負責調度爬蟲與特徵
    ...
    for ticker_code, name in tickers.items():
        ...
        # --- 嘗試 2：啟動終極官方備援機制 (美國聯準會 FRED 資料庫) ---
        if price_df is None or price_df.empty:
            # 建立 FRED 專屬的指標對照表
            fred_map = {
                "BZ=F": "DCOILBRENTE", # 布蘭特原油現貨
                "USDTWD=X": "DEXTAUS", # 台幣匯率 (Taiwan New Dollar)
                "JPY=X": "DEXJPUS",    # 日圓匯率
                "KRW=X": "DEXKOUS",    # 韓元匯率
                "^VIX": "VIXCLS"       # CBOE 恐慌指數
            }
            
            fred_id = fred_map.get(ticker_code)
            logging.warning(f"   ⚠️ 啟動美國聯準會(FRED)官方資料庫救援: {name} ({fred_id})...")
            
            try:
                # 直接請求 FRED 官方公開的 CSV 端點，完美避開所有反爬蟲防火牆
                fred_url = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={fred_id}"
                res = session.get(fred_url, timeout=15)
                
                # 確保成功回傳且內容包含日期欄位
                if res.status_code == 200 and "DATE" in res.text:
                    from io import StringIO
                    # ⚠️ FRED 針對假日沒有交易的日子，會填入 '.'，我們必須將其視為空值並剔除
                    df_fred = pd.read_csv(StringIO(res.text), na_values=['.'])
                    df_fred = df_fred.dropna()
                    df_fred['DATE'] = pd.to_datetime(df_fred['DATE'])
                    
                    # 過濾我們需要的日期範圍
                    df_fred = df_fred[df_fred['DATE'] >= pd.to_datetime(start_date)]
                    
                    if not df_fred.empty:
                        # 整理成後續特徵工程需要的格式
                        price_df = df_fred[['DATE', fred_id]].copy()
                        price_df.columns = ['日期', name]
                        logging.info(f"   ✅ FRED 救援大成功: {name}")
                    else:
                        logging.warning(f"   ⚠️ FRED 有回應，但期間內無有效數值")
                else:
                    logging.error(f"   ❌ FRED 回傳異常 (狀態碼: {res.status_code})")
            except Exception as e:
                logging.error(f"   ❌ FRED 備用源也失效: {e}")

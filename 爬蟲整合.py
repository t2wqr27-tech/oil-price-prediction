...
def build_refined_dataset(asia_realtime=None):  # 定義主資料集建構函式，負責調度爬蟲與特徵
    ...
    for ticker_code, name in tickers.items():
        ...
        # --- 嘗試 2：啟動 Web Proxy 代理備援機制 (完美繞過 IP 封鎖) ---
        if price_df is None or price_df.empty:
            logging.warning(f"   ⚠️ 官方管道阻擋，啟動公共代理節點救援: {name}...")
            import time
            import urllib.parse
            from io import StringIO
            
            # 將時間轉換為 Yahoo CSV 專用的 UNIX Timestamp
            start_ts = int(pd.to_datetime(start_date).timestamp())
            end_ts = int(time.time()) + 86400 # 加上一天確保時間涵蓋到最新
            yahoo_csv_url = f"https://query1.finance.yahoo.com/v7/finance/download/{ticker_code}?period1={start_ts}&period2={end_ts}&interval=1d&events=history"
            encoded_url = urllib.parse.quote(yahoo_csv_url, safe='')
            
            # 使用多個免費的公共 CORS 代理 (讓請求看起來來自世界各地的正常伺服器)
            proxy_endpoints = [
                f"https://api.allorigins.win/raw?url={encoded_url}",
                f"https://corsproxy.io/?{encoded_url}",
                f"https://thingproxy.freeboard.io/fetch/{yahoo_csv_url}"
            ]
            
            for p_url in proxy_endpoints:
                try:
                    res = session.get(p_url, timeout=15)
                    # 確保回傳內容是 CSV 且包含 Date 欄位
                    if res.status_code == 200 and "Date" in res.text:
                        # Yahoo CSV 有時假日會填入 'null'，這裡直接幫你清洗掉
                        df_proxy = pd.read_csv(StringIO(res.text), na_values=['null', 'NaN'])
                        df_proxy = df_proxy.dropna(subset=['Close'])
                        
                        if not df_proxy.empty:
                            df_proxy['Date'] = pd.to_datetime(df_proxy['Date'])
                            df_proxy = df_proxy[df_proxy['Date'] >= pd.to_datetime(start_date)]
                            
                            price_df = df_proxy[['Date', 'Close']].copy()
                            price_df.columns = ['日期', name]
                            logging.info(f"   ✅ 代理節點救援成功: {name}")
                            break  # 抓到資料就提早結束迴圈
                except Exception:
                    continue  # 代理失敗就換下一個節點
            
            if price_df is None or price_df.empty:
                logging.error(f"   ❌ 所有代理節點皆失效 ({name})")

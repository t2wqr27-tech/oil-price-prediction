...
def build_refined_dataset(asia_realtime=None):  # 定義主資料集建構函式，負責調度爬蟲與特徵
    ...
    # 建立備用資料庫 Stooq 的代碼對照表
    stooq_map = {
        "BZ=F": "CB.F",        # 原油 (Commodity - Brent)
        "USDTWD=X": "USDTWD.V", # 台幣
        "JPY=X": "USDJPY.V",   # 日圓
        "KRW=X": "USDKRW.V",   # 韓元
        "^VIX": "VIX.US"       # 恐慌指數
    }
    ...
    print("📦 嘗試批量下載 Yahoo Finance 首選資料...")
    ticker_list = list(tickers.keys())
    # 稍微縮短 Timeout，讓它被擋時能趕快切換到備用源
    bulk_data = yf.download(ticker_list, start=start_date, progress=False, timeout=10)
    ...
    for ticker_code, name in tickers.items():
        ...
        if not bulk_data.empty:
            try:
                ...
                if not price_series.empty:
                    ...
                    price_df.index.name = '日期'
                    price_df = price_df.reset_index()
                    print(f"   ✅ Yahoo 下載成功: {name}")
            except Exception:
                pass # Yahoo 處理失敗，安靜進入備用源流程
                
        # --- 嘗試 2：啟動 Stooq 備援機制 ---
        if price_df is None or price_df.empty:
            stooq_code = stooq_map.get(ticker_code)
            print(f"   ⚠️ Yahoo 失敗，切換 Stooq 備用源: {name} ({stooq_code})...")
            try:
                # 使用 pandas_datareader 從 Stooq 抓取
                ...
                if not df_stooq.empty:
                    ...
                    price_df.index.name = '日期'
                    price_df = price_df.reset_index()
                    print(f"   ✅ Stooq 備用源救援成功: {name}")
            except Exception as e:
                print(f"   ❌ Stooq 備用源也失敗: {e}")
        ...
        if price_df is not None and not price_df.empty:
            ...
            if name == '布蘭特原油' and (last_val > 200 or last_val < 10):
                print(f"   ⚠️ {name} 數值異常 ({last_val})，自動捨棄")
                continue

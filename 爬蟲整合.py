...
def build_refined_dataset(asia_realtime=None):  # 定義主資料集建構函式，負責調度爬蟲與特徵
    ...
    print("📦 啟動大包裝 (Bulk Download) 模式，僅發送 1 次 API 請求...")
    try:
        ticker_list = list(tickers.keys())
        # 利用 yfinance 的特性，將所有標的放在一個 list 裡一次下載
        bulk_data = yf.download(ticker_list, start=start_date, progress=False, timeout=20)
        
        if bulk_data.empty:
            print("❌ 合併下載失敗，資料為空。")
        else:
            # 將大包裝的 DataFrame 拆解，並餵進你原本設計好的防呆邏輯
            for ticker_code, name in tickers.items():
                print(f"   🔍 正在從大表中萃取: {name} ({ticker_code})...")
                ...
                if price_series.empty:
                    print(f"   ⚠️ {name} 查無有效歷史資料")
                    continue
                    
                # --- 轉回你原本架構需要的 DataFrame 格式 ---
                ...
                if name == '布蘭特原油' and (last_val > 200 or last_val < 10):
                    ...
                    continue
                elif name == '台幣匯率' and (last_val > 50 or last_val < 20):
                    print(f"   ⚠️ {name} 數值異常 ({last_val})，自動捨棄")
                    continue
                    
                print(f"   ✅ 萃取成功: {name} (最新值: {last_val:.2f})")
                new_data_frames.append(price_df)
                
    except Exception as e:
        print(f"❌ 批量下載過程中發生嚴重錯誤: {e}")
